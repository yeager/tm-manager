...
from tm_manager.accessibility import AccessibilityManager
                   import_ts, export_po)
...
class TMManagerWindow(Adw.ApplicationWindow):
    ...
    def _on_export_format_chosen(self, dialog, response):
        ...
        fd = Gtk.FileDialog()
        fd.set_initial_name(f"tm-export.{response}")
    ...
    def _on_export_save(self, dialog, result):
        try:
            ...
            return
        data = [{"source": u.source, "target": u.target,
                 "source_lang": u.source_lang, "target_lang": u.target_lang,
                 "project": u.project, "created": u.created}
                for u in self.filtered_units]
        if not data:
            return
...
if __name__ == "__main__":
    ...
    def _show_welcome(self, win):
        ...
        page.set_title(_("Welcome to TM Manager"))
        page.set_description(_("Manage translation memory databases.\n\n✓ Import .tmx and .po files\n✓ Search translation memories\n✓ Merge TM databases"))
